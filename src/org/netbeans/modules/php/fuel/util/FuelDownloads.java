...
public class FuelDownloads {
    ...
    public String[] getDownloadVersions() {
        return DOWNLOAD_VERSIONS.toArray(new String[0]);
    }
    ...
    private void initDownloads() {
        ...
        try {
            ...
            for (Element tr : downloads) {
                String version = tr.child(0).text();
                Elements urls = tr.select("a"); // NOI18N
                String url = ""; // NOI18N
                for (Element a : urls) {
                    url = a.attr("href"); // NOI18N
                    break;
                }
                if (!url.isEmpty() && url.startsWith(DL_URL_PREFIX)) {
                    DOWNLOAD_VERSIONS.add(version);
                    DOWNLOADS_MAP.put(version, url);
                }
